...
############### Part-7 Combining K-Fold and Cross Validation ##################
...
for train_index, test_index in kf.split(X):
   ...
   y_pred = grid_search.predict(X_test) # gives the probability
   test_score.append(accuracy_score(y_test, y_pred, normalize=True)*100)
   

##################### Part-8 Designing the Best Model #########################

def best_classifier(parameters):
    optimizer = best_parameters[0]['optimizer']
    b_size = best_parameters[0]['batch_size']
    epochs = best_parameters[0]['epochs']
    classifier = Sequential()
    classifier.add(Dense(units = 6, kernel_initializer='uniform', activation = 'relu',input_dim = 11))
    classifier.add(Dense(units = 6, kernel_initializer='uniform', activation = 'relu'))
    classifier.add(Dense(units = 1, kernel_initializer='uniform', activation = 'sigmoid')) # if the ouput category is more than 2 it will be softmax
    classifier.compile(optimizer = optimizer, loss = 'binary_crossentropy', metrics = ['accuracy'])
    classifier.fit(X, y, batch_size = b_size, epochs= epochs)
    return classifier

best_fit_model,optimizer,b_size,epochs = best_classifier(best_parameters)
